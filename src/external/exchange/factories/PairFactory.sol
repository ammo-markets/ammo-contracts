...
contract PairFactory is IPairFactory {
    ...
    modifier onlyGovernance() {
        if (msg.sender != address(accessHub)) revert Errors.NOT_AUTHORIZED(msg.sender);
        _;
    }
    ...
    function setFee(uint256 _fee) external onlyGovernanceOrVoter {
        ...
        if (_fee == 0) revert Errors.ZERO_FEE();
        /// @dev ensure less than or equal to MAX_FEE
        if (_fee > MAX_FEE) revert Errors.FEE_TOO_HIGH();
    ...
    function setFeeSplit(uint256 _feeSplit) external onlyGovernanceOrVoter {
        ...
        if (_feeSplit > FEE_DENOM) revert Errors.INVALID_FEE_SPLIT();
    ...
    function createPair(address tokenA, address tokenB, bool stable) external returns (address pair) {
        ...
        if (tokenA == tokenB) revert Errors.INVALID_ASSORTMENT();
        ...
        if (getPair[token0][token1][stable] != address(0)) revert Errors.PAIR_EXISTS();
    ...
    function setFeeRecipient(address _pair, address _feeRecipient) external {
        ...
        if (msg.sender != voter && msg.sender != address(accessHub)) revert Errors.NOT_AUTHORIZED(msg.sender);
