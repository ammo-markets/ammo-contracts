...
import {IExchangeWETH} from "./interfaces/IWETH.sol";
...
contract Router is IRouter {
    ...
    function sortTokens(address tokenA, address tokenB) public pure returns (address token0, address token1) {
        if (tokenA == tokenB) revert Errors.IDENTICAL();
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        if (token0 == address(0)) revert Errors.ZERO_ADDRESS();
    }
    ...
    {
        if (amountA == 0) revert Errors.INSUFFICIENT_AMOUNT();
        if (reserveA == 0 || reserveB == 0) revert Errors.INSUFFICIENT_LIQUIDITY();
    ...
    function getAmountsOut(uint256 amountIn, route[] memory routes) public view returns (uint256[] memory amounts) {
        if (routes.length < 1) revert Errors.INVALID_PATH();
    ...
    {
        if (amountOut == 0) revert Errors.INSUFFICIENT_OUTPUT_AMOUNT();
        ...
        uint256 decimalsIn;
        {
            ...
            if (reserve0 == 0 || reserve1 == 0) revert Errors.INVALID_RESERVES();
    ...
    function _addLiquidity(
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint256 amountBOptimal = quoteLiquidity(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                if (amountBOptimal < amountBMin) revert Errors.INSUFFICIENT_B_AMOUNT();
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = quoteLiquidity(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                if (amountAOptimal < amountAMin) revert Errors.INSUFFICIENT_A_AMOUNT();
                (amountA, amountB) = (amountAOptimal, amountBDesired);
            }
        }
    }
    ...
    function addLiquidityETH(
        ...
        IExchangeWETH(WETH).deposit{value: amountETH}();
        assert(IExchangeWETH(WETH).transfer(pair, amountETH));
        liquidity = IPair(pair).mint(to);
    ...
    function addLiquidityAndStake(
        ...
        if (gauge == address(0)) revert Errors.NO_GAUGE(pair);
    ...
    function removeLiquidity(
        ...
        if (amountA < amountAMin) revert Errors.INSUFFICIENT_A_AMOUNT();
        if (amountB < amountBMin) revert Errors.INSUFFICIENT_B_AMOUNT();
    }
    ...
    function removeLiquidityETH(
        ...
        IExchangeWETH(WETH).withdraw(amountETH);
    ...
    function swapExactTokensForTokens(
        ...
        if (amounts[amounts.length - 1] < amountOutMin) revert Errors.INSUFFICIENT_OUTPUT_AMOUNT();
        _safeTransferFrom(
    ...
    function swapTokensForExactTokens(
        ...
        if (amounts[0] > amountInMax) revert Errors.EXCESSIVE_INPUT_AMOUNT();
        _safeTransferFrom(
            routes[0].from, msg.sender, pairFor(routes[0].from, routes[0].to, routes[0].stable), amounts[0]
        );
        _swap(amounts, routes, to);
    ...
    {
        if (routes[0].from != WETH) revert Errors.INVALID_PATH();
        amounts = getAmountsOut(msg.value, routes);
        if (amounts[amounts.length - 1] < amountOutMin) revert Errors.INSUFFICIENT_OUTPUT_AMOUNT();
        IExchangeWETH(WETH).deposit{value: amounts[0]}();
        assert(IExchangeWETH(WETH).transfer(pairFor(routes[0].from, routes[0].to, routes[0].stable), amounts[0]));
        _swap(amounts, routes, to);
    }
    ...
    function swapTokensForExactETH(
        ...
        if (routes[routes.length - 1].to != WETH) revert Errors.INVALID_PATH();
        amounts = getAmountsIn(amountOut, routes);
        if (amounts[0] > amountInMax) revert Errors.EXCESSIVE_INPUT_AMOUNT();
        _safeTransferFrom(
            routes[0].from, msg.sender, pairFor(routes[0].from, routes[0].to, routes[0].stable), amounts[0]
        ...
        IExchangeWETH(WETH).withdraw(amounts[amounts.length - 1]);
        ...
        returns (uint256[] memory amounts)
    {
        if (routes[0].from != WETH) revert Errors.INVALID_PATH();
        amounts = getAmountsIn(amountOut, routes);
        if (amounts[0] > msg.value) revert Errors.EXCESSIVE_INPUT_AMOUNT();
        IExchangeWETH(WETH).deposit{value: amounts[0]}();
        assert(IExchangeWETH(WETH).transfer(pairFor(routes[0].from, routes[0].to, routes[0].stable), amounts[0]));
        _swap(amounts, routes, to);
        /// @dev refund dust eth, if any
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        if (IERC20Extended(routes[routes.length - 1].to).balanceOf(to) - balanceBefore < amountOutMin) {
            revert Errors.INSUFFICIENT_OUTPUT_AMOUNT();
        }
    }
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        if (routes[0].from != WETH) revert Errors.INVALID_PATH();
        IExchangeWETH(WETH).deposit{value: msg.value}();
        assert(IExchangeWETH(WETH).transfer(pairFor(routes[0].from, routes[0].to, routes[0].stable), msg.value));
        uint256 balanceBefore = IERC20Extended(routes[routes.length - 1].to).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(routes, to);
        if (IERC20Extended(routes[routes.length - 1].to).balanceOf(to) - balanceBefore < amountOutMin) {
            revert Errors.INSUFFICIENT_OUTPUT_AMOUNT();
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        if (amountOut < amountOutMin) revert Errors.INSUFFICIENT_OUTPUT_AMOUNT();
        IExchangeWETH(WETH).withdraw(amountOut);
    ...
    function _safeTransferETH(address to, uint256 value) internal {
        ...
        if (!success) revert Errors.ETH_TRANSFER_FAILED();
    }
