...
contract Pair is IPair, ERC20, ReentrancyGuard {
    ...
    modifier onlyFactory() {
        ...
        if (msg.sender != factory) revert Errors.NOT_AUTHORIZED(msg.sender);
        _;
    }
    ...
    function _update(uint256 balance0, uint256 balance1, uint112 _reserve0, uint112 _reserve1) private {
        ...
        if (balance0 > type(uint112).max || balance1 > type(uint112).max) revert Errors.OVERFLOW();
    ...
    function mint(address to) external nonReentrant returns (uint256 liquidity) {
        ...
        if (_totalSupply == 0) {
            ...
            if (stable) {
                if (_k(amount0, amount1) < MINIMUM_K) revert Errors.K();
                if ((amount0 * 1e18) / decimals0 != (amount1 * 1e18) / decimals1) {
                    revert Errors.UNSTABLE_RATIO();
                }
            }
        } else {
            liquidity = Math.min((amount0 * _totalSupply) / _reserve0, (amount1 * _totalSupply) / _reserve1);
        }
        if (liquidity == 0) revert Errors.INSUFFICIENT_LIQUIDITY_MINTED();
        _mint(to, liquidity);
    ...
    function burn(address to) external nonReentrant returns (uint256 amount0, uint256 amount1) {
        ...
        if (amount0 == 0 || amount1 == 0) revert Errors.INSUFFICIENT_LIQUIDITY_BURNED();
    ...
    function swap(uint256 amount0Out, uint256 amount1Out, address to, bytes calldata data) external nonReentrant {
        ...
        if (amount0Out == 0 && amount1Out == 0) revert Errors.INSUFFICIENT_OUTPUT_AMOUNT();

        /// @dev gas savings
        (uint112 _reserve0, uint112 _reserve1,) = getReserves();
        /// @dev ensure there is enough liquidity for the swap
        if (amount0Out >= _reserve0 || amount1Out >= _reserve1) revert Errors.INSUFFICIENT_LIQUIDITY();
        ...
        /// @dev stack-too-deep
        {
            ...
            if (to == _token0 || to == _token1) revert Errors.INVALID_TRANSFER();
            ...
            if (amount0In == 0 && amount1In == 0) revert Errors.INSUFFICIENT_INPUT_AMOUNT();
            ...
            if (_k(balance0Adjusted, balance1Adjusted) < _k(uint256(_reserve0), uint256(_reserve1))) {
                revert Errors.K();
            }
        }
    ...
    function skim(address to) external nonReentrant {
        ...
        if (!IPairFactory(factory).skimEnabled(address(this))) revert Errors.SKIM_DISABLED();
        /// @dev gas savings
        address _token0 = token0;
        /// @dev gas savings
        address _token1 = token1;
